...
summaryStr = sprintf( ...
    'Original\n  %s – %s\n  dur: %.2f s\n  f:   [%.0f %.0f] Hz', ...
    tOrigStart, tOrigEnd, tOrigDur, freqOrig(1), freqOrig(2));
summaryStr = [summaryStr, sprintf( ...
    '\n\nTrimmed\n  %s – %s\n  dur: %.2f s\n  f:   [%.0f %.0f] Hz', ...
    tTrimStart, tTrimEnd, tTrimDur, freqTrimmed(1), freqTrimmed(2))];
if ~isempty(snrBefore) && ~isempty(snrAfter)
    summaryStr = [summaryStr, sprintf( ...
        '\n\nSNR (spectrogramSlices)\n  Before: %.1f dB\n  After:  %.1f dB', ...
        snrBefore, snrAfter)];
end
text(ax4, 0.05, 0.95, summaryStr, 'Units', 'normalized', ...
    'FontSize', 7.5, 'VerticalAlignment', 'top', 'FontName', 'FixedWidth');
