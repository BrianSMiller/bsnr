...
text(ax, ndb, yMax * 0.92, sprintf('Noise\n%.1f', ndb), ...
...
text(ax, sdb, yMax * 0.92, sprintf('Signal\n%.1f', sdb), ...
...
text(ax, mean([ndb sdb]), yMax * 0.99, ...
...
title(ax, 'NIST — frame energy distribution', 'FontSize', 7);
hold(ax, 'off');
