...
arguments
    annot
    options.snrType            char     = 'spectrogram'
    options.freq                        = []
    options.nfft                        = []
    options.nOverlap                    = []
    ...
    options.ridgeParams        struct   = struct()
    options.resampleRate                = []
...
% metadata -> calibration alias
if ~isempty(options.metadata) && isempty(options.calibration)
    options.calibration = options.metadata;
end

% removeClicks sub-field defaults
if ~isempty(options.removeClicks)
    if ~isfield(options.removeClicks, 'threshold')
        options.removeClicks.threshold = 3;
    end
    if ~isfield(options.removeClicks, 'power')
        options.removeClicks.power = 1000;
    end
end
