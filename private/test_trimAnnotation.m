...
% Build a batch just above parallelThreshold using the same fixture
nParTest  = 12;   % above default threshold of 10 (using parallelThreshold=10 for test)
annotBatch = repmat(annotWide, nParTest, 1);

trimSerial   = trimAnnotation(annotBatch, 'freq', freq, 'parallelThreshold', nParTest+1);
trimParallel = trimAnnotation(annotBatch, 'freq', freq, 'parallelThreshold', nParTest-1);

assert(numel(trimSerial)   == nParTest, 'serial: wrong output size');
assert(numel(trimParallel) == nParTest, 'parallel: wrong output size');

% t0, tEnd, freq should match within floating point
