...
fprintf(['\n--- Reproducibility notes ---\n' ...
    'bsnr does not exactly reproduce the SNR values in the Miller et al. (2022)\n' ...
    'capture history (Data S4). Best bsnr match: spectrogram [20-115] before\n' ...
    'Lurton (r=0.604, RMSE=11.8 dB, bias=+2.4 dB).\n' ...
    '\n' ...
    'The supplemental MATLAB code (Code S3, judgeDetections.m + annotationSNR.m)\n' ...
    'also does not reproduce the capture history SNR column when run on the same\n' ...
    'input data (r=0.393, RMSE=14.26 dB). Possible causes:\n' ...
    '  - c.duration = max(analyst, detector) in addSnr() propagates NaN for\n' ...
    '    detector-only detections, affecting noise window lengths\n' ...
    '  - t0 averaging for consensus detections (analyst + detector both true\n' ...
    '    positive) may have been applied before SNR was computed\n' ...
    '  - annotationSNR.m uploaded to supplemental may differ from version used\n' ...
    '    to generate results (common during peer review)\n' ...
    '\n' ...
    'The key validation result holds regardless: bsnr analyst SNR bin boundaries\n' ...
    '(-10.8 and -2.2 dB) are consistent with the paper''s reported values\n' ...
    '(-12 and -3 dB) within 2 dB, and the detector median SNR is lower than\n' ...
    'the analyst median SNR, replicating the paper''s central finding.\n' ...
    '\n' ...
    'TODO: Check annotatedLibrary GitHub commit history for a version of\n' ...
    'annotationSNR closer to the one that produced the paper results.\n' ...
    'Also investigate whether analyst+detector t0/duration were averaged for\n' ...
    'consensus true positives before SNR was computed.\n']);
