...
noiseLocationStrategy = 'beforeAndAfter';
...
for s = 1:nSites
    ...
    for c = 1:numel(callTypeNames)
        ...
        snrParams = struct( ...
            ...
            'nfft',          nfft, ...
            'nOverlap',      nOverlap, ...
            'noiseLocation', noiseLocationStrategy, ...
...
title(sprintf('bsnr  (%s, noiseDelay=%.0f s, Lurton, [%d-%d] Hz)', ...
    noiseLocationStrategy, noiseDelaySeconds, signalFrequencyBand(1), signalFrequencyBand(2)));
...
fprintf('\nResults saved to: %s\n', outputFile);
fprintf('Settings: noiseLocation=%s, noiseDelay=%.1f s, freq=[%d %d] Hz, useLurton=true\n', ...
    noiseLocationStrategy, noiseDelaySeconds, signalFrequencyBand(1), signalFrequencyBand(2));
